...
+
. A new menu called *Clone template* will appear, name the template `centos-stream9-server-db-xl`, select `vmexamples` as the template project, and set the display name to `CentOS Stream 9 VM - Database XL Template`, when finished click the `Clone` button.
+
...
+
. A new window will pop out where you can edit the amount of CPU and Memory. For our XL template set the value of CPUs to 4, and Memory to 16 GiB.
+
...
+
. When the Cloud-init dialog options click the radio button to *Configure via: Script* then copy in the following YAML snippet to the end of the script.
+
----
packages:
  - mariadb-server
...
+
. When the virtual machine boots you can see on the *Overview* page that it was created from our template, and has the system resources we defined, and is already attached to the vmexamples/vlan0 network.
+
...
+
. Once you are logged into the virtual machine, run the following command to test our install of MariaDB.
+
----
sudo mysql -u root
----
...
+
. Once the VM installation process is complete we can use a simliar process to the one at the beginning of this chapter to save it as a template that we can easily deploy in the future.

== Summary

In this section we learned how to clone and customize and existing template to create one that can be used for specific workloads like databases. We also learned how to configure one of the existing Windows templates without a boot source, and automate it's installation process.
