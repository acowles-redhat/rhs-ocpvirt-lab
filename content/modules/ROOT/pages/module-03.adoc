...
For this section of the lab we are going to create a *MultiNetwork Policy* that prevents all traffic by default from any other namespace from reaching our fedora01 VM, we are then going to apply another policy that allows connections only from a selected host, in this case fedora02.
...
In this section we learned a little bit more about how networking works in OpenShift Virtualization. We created a network attachment definition so that our VMs have network access from outside of the cluster. We also implemented a MultiNetwork Policy to help secure connections to our VMs by only allowing specified hosts. 
